...
describe 'Answers API', type: :request do
  ...
  describe 'GET /api/v1/questions/:question_id/answers' do
    it_behaves_like 'API Index' do
      ...
      let(:object_json) { json['answers'].first }
      let(:objects_json) { json['answers'] }
      let(:object) { answer }
      let(:object_attr) { %w[id body created_at updated_at] }
  ...
  describe 'GET /api/v1/questions/:question_id/answers/:id' do
    it_behaves_like 'API Show' do
      let(:api_url) { "/api/v1/questions/#{question.id}/answers/#{answer.id}" }
      let(:object_json) { json['answer'] }
      let(:object) { answer }
      let(:object_attr) { %w[id body created_at updated_at] }
    end
  end
  ...
  describe 'PATCH /api/v1/questions/:question_id/answers/:id' do
    it_behaves_like 'API Update Object' do
      let(:api_url) { "/api/v1/questions/#{question.id}/answers/#{answer_with_author.id}" }
      let(:object_json) { json['answer'] }
